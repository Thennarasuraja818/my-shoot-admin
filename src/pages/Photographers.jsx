...
import { Search, Star, Phone, Plus, UserPlus, ShieldAlert } from 'lucide-react';
...
const initialForm = { name: '', phone: '', email: '', specialization: 'Wedding', status: 'Available', shoots: 0, rating: 5.0 };
...
export default function Photographers() {
  ...
  const handleAdd = () => {
    ...
    setPhotographers(prev => [...prev, { id: Date.now(), ...form, shoots: Number(form.shoots), rating: Number(form.rating) }]);
    setAddOpen(false);
    setForm(initialForm);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 bg-[#141414] p-6 rounded-3xl border border-[#222]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => { setForm(initialForm); setAddOpen(true); }}
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="relative w-full max-w-md">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search team by name or specialization..."
        ...
        {filtered.map(ph => {
          ...
          return (
            <div key={ph.id} onClick={() => { setSelected(ph); setDetailOpen(true); }}
              className="bg-[#141414] border border-[#2a2a2a] rounded-2xl p-6 hover:border-[#333] hover:translate-y-[-4px] transition-all cursor-pointer group shadow-xl">
      ...
      {/* Onboard Modal */}
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Onboard New Team Member" size="sm">
