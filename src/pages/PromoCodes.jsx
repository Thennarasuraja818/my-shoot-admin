...
import { Search, Plus, Trash2, Ticket, Users, Calendar, Award, Power, Edit3 } from 'lucide-react';
...
export default function PromoCodes() {
  ...
  const [form, setForm] = useState({ code: '', type: 'Percentage', value: '', expiry: '', usageLimit: '', target: 'All Users', status: 'Active' });
  ...
  const handleAdd = () => {
    ...
    setAddOpen(false);
    setForm({ code: '', type: 'Percentage', value: '', expiry: '', usageLimit: '', target: 'All Users', status: 'Active' });
  };
        ...
        <div className="space-y-5 py-2">
           ...
           <div>
              ...
              {form.target === 'Selected Users' && (
                <div className="mt-4 p-4 bg-[#0c0c0c] border border-blue-500/20 rounded-2xl">
                   <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mb-3 flex items-center gap-2 italic">
                      <Award size={12} /> Reward logic enabled
                   </p>
                   <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                      {users.map(u => (
                        <div key={u.id} className="flex items-center gap-2 px-3 py-1.5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg text-[10px] font-medium text-gray-400">
                           <input type="checkbox" className="accent-[#f5b400]" /> {u.name}
                        </div>
                      ))}
                   </div>
                </div>
